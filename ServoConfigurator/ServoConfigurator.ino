...
uint16_t servoPulse[(4*SERVOS_COUNT)] =    {
                              SERVO_MIN_eyeLeftUD       ,
                              SERVO_MIN_eyeLeftLR       ,
...
void setup() {
  ...
  */
  ...
  writePulsesToDisplay.begin();

  #ifdef USE_PWM_DRIVER
    Serial.println("setup: pwd1.begin()");
    pwm1Available = pwm1.begin();
    if(pwm1Available) {
      Serial.println("setup: OK PCA9685 PWM1 connected");
    } else {
      Serial.println("setup: Looks like PCA9685 PWM1 driver is NOT connected!!!");
    }
    ...
    Serial.println("setup: pwd2.begin()");
    pwm2Available = pwm2.begin();
    if(pwm2Available) {
      Serial.println("setup: OK PCA9685 PWM2 connected");
    } else {
      Serial.println("setup: Looks like PCA9685 PWM2 driver is NOT connected!!!");
    }
  ...
  //------------------
   ...
   //Serial.println("setup: wait before loop----------------------------------------------");
   //delay(10000);
...
void loop() {
  ...
  loop_servoSet_BTN_Select(currentMillis);
      ...
      RotEnc_EvaluateIncrement(&myEncMax1, RotEnc_Row1_MAX, ROTARY_DIVIDER, activeServoSet, LEFT_ARROW_STEP, LABEL_FORM_MAX-1);
...
int16_t RotEnc_EvaluateIncrement(Encoder *myEnc, uint16_t encoderIndex, uint16_t divider, uint16_t active_ServoSet, uint16_t left_arrow_step, uint16_t Min_Mid_Max) {
    ...
    //--------------------------------------------------------------------------
    int16_t increment = 0;
    uint16_t servoPulseIndex = (active_ServoSet * left_arrow_step) + (SERVOS_COUNT * Min_Mid_Max);
...
void ReadHwData() {
  ...
  mydata_send.s1mid = servoPulse[ activeServoSet + (SERVOS_COUNT)];

  mydata_send.s1max = servoPulse[ activeServoSet + (SERVOS_COUNT*2)];

  mydata_send.s1curr= servoPulse[ activeServoSet + (SERVOS_COUNT*3)];
  
  mydata_send.devType =  3; // mode:  0 = fourSticksController (8 chanels) ,   1 = ServoConfigurator (16 chanels) , 2 = MinMaxServoConfig (min max for 2 chanels), 3 = MinMaxServoConfig for 18 servos
  //mydata_send.flags = 0;
    ...
    yPos = 2 + (activeServoSet*((8+2) * LEFT_ARROW_STEP));
        uint8_t i = 0;
          ...
          if(prevServoPulse[servoPulseIndex] != servoPulse[servoPulseIndex]) {
            data_changed = true;
            writePulsesToDisplay.writeMINPulsesToDisplay((activeServoSet*LEFT_ARROW_STEP)+i,servoPulse[servoPulseIndex], true);
            prevServoPulse[servoPulseIndex] = servoPulse[servoPulseIndex];
          }
          
          servoPulseIndex = (((activeServoSet*LEFT_ARROW_STEP)+i)) + (SERVOS_COUNT*1);
          if(prevServoPulse[servoPulseIndex] != servoPulse[servoPulseIndex]) {
            data_changed = true;
            writePulsesToDisplay.writeMIDPulsesToDisplay((activeServoSet*LEFT_ARROW_STEP)+i,servoPulse[servoPulseIndex], true);
            prevServoPulse[servoPulseIndex] = servoPulse[servoPulseIndex];
          }
          //-------------------------------------------------------------
          servoPulseIndex = (((activeServoSet*LEFT_ARROW_STEP)+i)) + (SERVOS_COUNT*3);
          if(prevServoPulse[servoPulseIndex] != servoPulse[servoPulseIndex]) {
            data_changed = true;
            #ifdef SEND_FROM_0_TO_255
              uint16_t origValue_0_255 = servoPulse[((servoIndexForAnalog) + (SERVOS_COUNT * 3))];
              uint16_t extrapolatedServoPulse_0_1023 = 
              (origValue_0_255)<128 ? 
                (map(origValue_0_255,   0, 127, servoPulse[(servoIndexForAnalog)           ], servoPulse[(servoIndexForAnalog) + (SERVOS_COUNT * 1)]) ) 
              : (map(origValue_0_255, 128, 255, servoPulse[(servoIndexForAnalog) + (SERVOS_COUNT * 1)], servoPulse[(servoIndexForAnalog) + (SERVOS_COUNT * 2)]) );

              writePulsesToDisplay.writeCurrPulsesToDisplay((activeServoSet*LEFT_ARROW_STEP)+i, extrapolatedServoPulse_0_1023);
            #else
              writePulsesToDisplay.writeCurrPulsesToDisplay((activeServoSet*LEFT_ARROW_STEP)+i,servoPulse[servoPulseIndex]);
            #endif
            prevServoPulse[servoPulseIndex] = servoPulse[servoPulseIndex];
          }

          //-------------------------------------------------------------
          servoPulseIndex = (((activeServoSet*LEFT_ARROW_STEP)+i)) + (SERVOS_COUNT*2);
          if(prevServoPulse[servoPulseIndex] != servoPulse[servoPulseIndex]) {
            data_changed = true;
            writePulsesToDisplay.writeMAXPulsesToDisplay((activeServoSet*LEFT_ARROW_STEP)+i,servoPulse[servoPulseIndex],true);
            prevServoPulse[servoPulseIndex] = servoPulse[servoPulseIndex];
          }
...
void loop_servoSet_BTN_Select(unsigned long currentMillis){
  if (currentMillis - previousMillis_BTN_Select >= interval_BTN_Select) {  // start timed event for read and send
    ...
    if (upButtonState == LOW){
      ...
      writePulsesToDisplay.writeArrow_activeServoSet(activeServoSet);
      delay(150);
    }
